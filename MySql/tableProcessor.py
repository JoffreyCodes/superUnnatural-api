...
def runLocationsProcessor():
    with open(INPUT_LOCATIONS_FILE, mode='r', encoding=encode) as csv_file:
        ...
        for row in reader:
            ...
            for i in range(len(row)):
                ...
                if colNames[i] == 'Near the Ground':                    
                    isTrue = row[i] == 'checked'
                    locationEntry[colNames[i]] = True if isTrue else False
                else:
...
def getHeaders():
    headers = {}
    with open(INPUT_WORKOUTS_FILE, mode='r', encoding=encode) as csv_file:
...
def runProcessor():
    ...
    with open(INPUT_WORKOUTS_FILE, mode='r', encoding=encode) as csv_file:
        ...
        for row in reader:
            ...
            handleSongMapLocationList(row, 'songlocations')
...
def createCSV(filename, headers, items):
    with open(PATH + 'CSV/' + filename, "w", newline="", encoding=encode) as f:
        writer = csv.DictWriter(f, headers)
        writer.writeheader()
        writer.writerows(items)

def createJSON(filename, items):
    with open(PATH + 'JSON/' + filename + '.json', "w", encoding='utf-8') as out:
...
createInstrxn = {
    'workouts': {
        ...
        'headers' : [
            ...
            'SN ID (from Coach)', 'SN ID (from Intensity)', 'Max Targets', 
            'Max Dodges', 'Triangles', 'Max Knee Strikes', 'Max Score', 
